          ...
          >
            ...
            <div className="px-8 py-16">
              <div className={`${stackedLayout ? "max-w-4xl mx-auto space-y-12" : "grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-7xl mx-auto"}`}>
                ...
                <div className="space-y-8">
                  {id === "slack-combined" ? (
                    /* Custom video layout for Slack combined panel */
                      ...
                      {isMobile ? (
                        ...
                        <div className="space-y-8">
                          {videoAssets.map((asset, index) => {
                            const assetMetadata = {
                              effectiveTitle: asset.title || title,
                              effectiveSubtitle: asset.subtitle || subtitle,
                              effectiveDescription: asset.description || description,
                              effectiveRole: asset.role || role,
                              effectiveClient: asset.client || client,
                              effectiveAgency: asset.agency || agency,
                              effectiveProductionCompany: asset.productionCompany || productionCompany,
                              effectiveAdditionalCredits: asset.additionalCredits || additionalCredits,
                              effectiveExternalLinks: asset.externalLinks || externalLinks
                            };
                              ...
                              <div key={index} className="space-y-4">
                                ...
                                <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                                  <div className="flex items-center space-x-3 mb-2">
                                    <span className="text-sm font-medium text-white/70 bg-white/10 px-3 py-1 rounded-full" data-testid={`text-video-counter-${index}`}>
                                      Video {index + 1} of {videoAssets.length}
                                    </span>
                                  </div>
                                  <h4 className="text-xl font-semibold text-white" data-testid={`text-video-title-${index}`}>
                                    {assetMetadata.effectiveSubtitle}
                                  </h4>
                                  <p className="text-white/80 mt-2 leading-relaxed">
                                    {assetMetadata.effectiveDescription}
                                  </p>
                                </div>
                                ...
                                <div className="relative bg-gray-900 rounded-lg overflow-hidden" style={{ aspectRatio: '16/9' }}>
                                  <VideoThumbnail
                                    ...
                                    aspectRatio={asset.aspectRatio}
                                    onExpand={() => {
                                      const assetIndex = videoAssets.indexOf(asset);
                                      handleVideoExpand(assetIndex);
                                    }}
                                  />
                                </div>
                              </div>
                        ...
                        /* Desktop: Video switching UI */
                        <div className="space-y-6">
                          {/* Current Video Header with Counter */}
                          <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                            <div className="flex items-center justify-between">
                              <div>
                                <div className="flex items-center space-x-3 mb-2">
                                  <span className="text-sm font-medium text-white/70 bg-white/10 px-3 py-1 rounded-full" data-testid="text-video-counter">
                                    Video {variantIndex + 1} of {videoAssets.length}
                                  </span>
                                  <span className="text-sm text-white/70">Now Playing</span>
                                </div>
                                <h4 className="text-xl font-semibold text-white" data-testid="text-current-video-title">
                                  {effectiveSubtitle}
                                </h4>
                              </div>
                            </div>
                          ...
                          {/* Video Selection Tabs - Above Video */}
                          <div className="flex space-x-2">
                            {videoAssets.map((asset, index) => (
                              <button
                                key={index}
                                onClick={() => setVariantIndex(index)}
                                className={`flex-1 p-3 rounded-lg text-sm font-medium transition-all duration-300 ${
                                  index === variantIndex 
                                    ? 'bg-accent/20 border border-accent text-white' 
                                    : 'bg-white/5 border border-white/10 text-white/70 hover:bg-white/10 hover:text-white'
                                }`}
                                data-testid={`button-variant-${index}`}
                                aria-label={`Switch to ${asset.subtitle}`}
                              >
                                <div className="text-center">
                                  <div className="text-xs text-white/60 mb-1">Video {index + 1}</div>
                                  <div className="font-medium">{asset.subtitle?.replace('Slack ', '') || `Video ${index + 1}`}</div>
                                </div>
                              </button>
                            ))}
                          </div>
                          ...
                          <div className="relative bg-gray-900 rounded-lg overflow-hidden" style={{ aspectRatio: '16/9' }}>
                            <VideoThumbnail
                              videoUrl={videoAssets[variantIndex]?.videoUrl}
                              thumbnailUrl={videoAssets[variantIndex]?.thumbnailUrl || ""}
                              aspectRatio={videoAssets[variantIndex]?.aspectRatio}
                              onExpand={() => handleVideoExpand(variantIndex)}
                            />
                          </div>
                          
                          {/* Video Navigation Controls - Below Video */}
                          <div className="flex items-center justify-center space-x-4">
                            {variantIndex > 0 && (
                              <button
                                onClick={() => setVariantIndex(variantIndex - 1)}
                                className="flex items-center space-x-2 px-4 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-white transition-all duration-300 hover:scale-105"
                              ...
                              >
                                ...
                                <span className="text-sm font-medium">Previous Video</span>
                              </button>
                            )}
                            
                            {variantIndex < videoAssets.length - 1 && (
                              <button
                                onClick={() => setVariantIndex(variantIndex + 1)}
                                className="flex items-center space-x-2 px-4 py-2 bg-accent/20 border border-accent/30 hover:bg-accent/30 rounded-lg text-white transition-all duration-300 hover:scale-105"
                                data-testid="button-next-variant"
                                aria-label="Switch to next video"
                              >
                                <span className="text-sm font-medium">
                                  Next: {videoAssets[variantIndex + 1]?.subtitle?.replace('Slack ', '') || 'Next Video'}
                                </span>
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                                </svg>
                    ...
                    /* Custom video layout for Nintendo combined panel */
                    <div className="relative">
                      {isMobile ? (
                        /* Mobile: Stack both videos vertically */
                        <div className="space-y-8">
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="flex space-x-2">
                            {videoAssets.map((asset, index) => (
                              ...
                              >
                                <div className="text-center">
                                  <div className="text-xs text-white/60 mb-1">Video {index + 1}</div>
                                  <div className="font-medium">{asset.subtitle || `Video ${index + 1}`}</div>
                                </div>
                              </button>
                            ))}
                          </div>
                          ...
                          <div className="relative bg-gray-900 rounded-lg overflow-hidden" style={{ aspectRatio: '16/9' }}>
                            <ReactPlayer
                              key={videoAssets[variantIndex]?.videoUrl} // Force remount on video change
                              url={videoAssets[variantIndex]?.videoUrl}
                              playing={false}
                              controls={true}
                              width="100%"
                              height="100%"
                          ...
                          <div className="flex items-center justify-center space-x-4">
                            ...
                            {variantIndex < videoAssets.length - 1 && (
                              ...
                              >
                                <span className="text-sm font-medium">
                                  Next: {videoAssets[variantIndex + 1]?.subtitle || 'Next Video'}
                                </span>
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                                </svg>
